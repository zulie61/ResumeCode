...
def which_num(occur):
    for j in range(10):
        ...
        # 判斷低頻區間的主頻率
        low_peak = fft.interval_majorPeak(13,23)
        # 判斷高頻區間的主頻率
        high_peak = fft.interval_majorPeak(23,33)
        # 判斷低頻與高頻落在哪個區間內
        for i in range(10):
            # 篩選位於哪個低頻閾值
            if low_peak < low_loss[i]:
                # 篩選出的低頻閾值中, 高頻聲音為何者
                if high_peak < high_loss[i]: 
                    occur[i] += 1 # 判斷出的數字出現次數 + 1
                    break
    # 輸出判斷最多次的按鍵聲號碼
    print((occur.index(max(occur))+1)%10)  
