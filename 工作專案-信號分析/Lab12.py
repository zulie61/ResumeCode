...
def init():
    ...
    for i in range(10):  # 執行代表數字'0'~'9'的速度
        ...
        init_hz.append(peak) # 將初始頻率加入串列內
    ...
    for j in range(9):
        # 相鄰兩項相加的平均
        conti_ave = (init_hz[j] + init_hz[j+1]) / 2  
...
def run_judge(ascii_unm,threshold): # 主程式中重複執行的部分
    ...
    motor.duty(400 + 50*ascii_unm) # ASCII碼個別數字運轉
    time.sleep(1) # 穩定轉速
...
while True:
    ...
    for i in range(len(data)):
        ...
        # 轉為 str 後才可以使用 len 函式來看長度
        for j in range(len(ascii_num)):
            # 轉回 int 作為運轉馬達的引數
            run_judge(int(ascii_num[j]),threshold)  
        print(end=' ') # 字與字之間加入空格
    print()